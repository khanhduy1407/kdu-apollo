import Kdu, { PluginObject, PluginFunction } from 'kdu'
import {
  ApolloClient,
  ObservableQuery,
  ApolloQueryResult,
...
import { ApolloProvider, KduApolloComponent } from './apollo-provider'
import {
...
export class KduApollo extends ApolloProvider implements PluginObject<{}>{
  [key: string]: any
  install: PluginFunction<{}>

  static install(pKdu: typeof Kdu, options?:{} | undefined): void
}
...
export interface DollarApollo<V> extends ApolloClientMethods {
  ...
  addSmartQuery<R = any>(key: string, options: KduApolloQueryDefinition<R>): SmartQuery<V>
  addSmartSubscription<R = any>(key: string, options: KduApolloSubscriptionDefinition): SmartSubscription<V>
}
