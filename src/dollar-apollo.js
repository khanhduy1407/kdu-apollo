...
export class DollarApollo {
  ...
  getClient (options = null) {
    if (!options || !options.client) {
      ...
      if (this.client) {
        if (!this.provider.clients) {
          throw new Error(`[kdu-apollo] Missing 'clients' options in 'apolloProvider'`)
        } else {
          const client = this.provider.clients[this.client]
          if (!client) {
            throw new Error(`[kdu-apollo] Missing client '${this.client}' in 'apolloProvider'`)
          }
          return client
        }
      }
    ...
    if (!client) {
      throw new Error(`[kdu-apollo] Missing client '${options.client}' in 'apolloProvider'`)
    }
    return client
  }
